...
def realtorregistration(request):
  if request.method == 'POST':
    ...
    if Realtor.objects.filter(name=name).exists():
      ...
      if User.objects.filter(email=email).exists():
        ...
        return redirect('register')
          ...
          user = Realtor.objects.create(name=name,email=email,description=description,is_mvp=is_mvp,photo=realtor_image)
          user.save()
          messages.success(request, 'You are now registered as Realtor')
      return redirect('home')
  else:
    return render(request, 'realtorregistration.html')
